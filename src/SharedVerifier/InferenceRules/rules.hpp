...
std::pair<bool, Assumptions> verify##N##ElimSemantics(const Proof& p, const VertId vertexId, const Assumptions& assumptions);\
bool verify##N##IntroSyntax(const Proof& p, const VertId vertexId);\
std::pair<bool, Assumptions> verify##N##IntroSemantics(const Proof& p, const VertId vertexId, const Assumptions& assumptions);\

bool verifyAssumptionSyntax(const Proof& p, const VertId vertexId);
std::pair<bool, Assumptions> verifyAssumptionSemantics(const Proof& p, const VertId vertexId, const Assumptions& assumptions);
